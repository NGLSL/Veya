...
16. 内存回归使用隔离数据库准备超过一页的图片历史，分别记录启动、翻页、打开/关闭完整图片弹窗，以及隐藏、最小化、设置页和重新打开历史页后的进程 Private Bytes 和 Working Set；确认首页只显示 25 张卡片，后台记录时不查询历史页，重新打开能读到新记录。默认 `wgpu` 渲染器与显式 `ICED_BACKEND=tiny-skia` 可用同一构建对照；软件渲染器仅供诊断，不作为窗口显示行为的验收依据。不要使用日常数据库作为压力测试夹具。
