...
2. `veya-desktop/src/worker.rs` 独占运行中的 `FlowEngine` 和 `Store`：`FlowEngine` 只保留当前粘贴关联所需的活跃记录，完整历史保存在 SQLite。worker 处理平台事件和 `WorkerCmd`，把变化写回 SQLite；历史按 `(created_at_ms, sequence)` 游标分批读取，每次只发布一页聚合卡片。搜索、分类、固定筛选和排序在分批读取时应用于完整历史。追踪开关的 UI 请求带序号；快照回传已处理的序号，避免定时同步把刚点击的状态短暂覆盖为旧值。
...
| `veya-storage` | `Store::open/count_records/load_records_page/load_record/insert_record/set_pinned/delete_records/purge_older_than`、设置和排除应用读写。负责旧文本库迁移、SQLite 载荷与固定状态持久化；不判断一次粘贴是否成功。 |
...
- **历史内存边界**：历史页每次显示 25 张卡片；worker 从 SQLite 分批读取并丢弃非当前页的载荷。窗口隐藏、最小化或显示设置页时，worker 不查询历史页并清空已发布卡片；重新显示历史页时再读取。图片卡片只保留最长边不超过 128 像素的缩略图，完整 PNG 仅在打开大图弹窗时按序列号读取。重新复制旧记录同样按序列号读取原始载荷。
